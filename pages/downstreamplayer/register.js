...
class DownstreamPlayerRegister extends Component {
  state = {
    inventory: '',
    errorMessage: '',
    loading: false
   ...
   onSubmit = async(event) => {
    ...
    .registerAsDownstreamPlayer(this.state.inventory)
    .send({
      from: currentAccount
    });
    console.log("registered successfully as downstream player!");
  }
  catch(err){
  ...
  render(){
    ...
    <h2> Please enter your current inventory below and register as downstream player</h2>
    ...
    <Input maxLength = "10" label = "units" labelPosition = "right" value = {this.state.inventory} onChange = {event => this.setState({inventory:event.target.value})}/>
      ...
      <Button primary floated = "left" loading = {this.state.loading} content ="Register as downstream player!" />
      </Form>
      </Layout>);
  };
...
export default DownstreamPlayerRegister;
